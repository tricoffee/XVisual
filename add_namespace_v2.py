...
def process_cpp_file(file_path):
    ...
    if not lines or all(line.strip().startswith('#include') or not line.strip() for line in lines):
        print(f"跳过（空文件或只有includes）: {file_path}")
        return False
    ...
    # 在最后一个include后、第一个非空代码前添加namespace开始
    ...
    for root, dirs, files in os.walk(base_path):
        for file in files:
            ...
            if file.endswith('.cpp'):
                ...
                result = process_cpp_file(file_path)
                if result:
                    processed_count += 1
                else:
                    error_count += 1
